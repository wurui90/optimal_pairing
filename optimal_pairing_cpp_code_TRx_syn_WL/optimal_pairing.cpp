...
//	std::normal_distribution<double> norm_dist(0.0, wl_std);
//	std::default_random_engine rng;
	boost::mt19937 *rng = new boost::mt19937();
	rng->seed(time(NULL));
	boost::normal_distribution<> distribution(0.0, wl_std);
	boost::variate_generator< boost::mt19937, boost::normal_distribution<> > norm_dist(*rng, distribution);
   ...
   // printf("Synthetic WLs:\n");
    ...
    for(int i = 0; i < TRX_NUM; i++)
    {
       // printf("Device#%d:", i);
        ...
        {
            TRx_WLs[i][j] = wl0 + channel_spacing*j + norm_dist();
            TRx_WLs[i][j + CHAN_NUM] = wl0 + channel_spacing*j + norm_dist();
...
*/	
    ...
    usedFlag[currPairIndex.second] = true;
       ...
       // printf("(%d, %d): %f\n", currPairIndex.first + 1, currPairIndex.second + 1, min_cost);

        printf("%f\n", min_cost);

	}
		printf("Local pairing cost (per pair):\n");
		double cost0 = 0;
		for(int i = 0; i < floor(TRX_NUM/2); i++)
			cost0 += costMat[2*i][2*i + 1];
		printf("%lf\n", cost0/floor(TRX_NUM/2));
