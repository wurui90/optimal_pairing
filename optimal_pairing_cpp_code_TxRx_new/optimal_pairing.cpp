...
//Tuning cost for Transceiver (TRx), with TRx1 and TRx2, each with 5x2 = 10 wavelengths 
...
int main(int argc, char** argv)
{
   ...
   // CHAN_NUM = atoi(argv[2]);
    ...
    for(int i = 0; i < TRX_NUM; i++)
    {
        ...
        {
            Tx_WLs[i][j] = wl0 + channel_spacing*j + norm_dist(rng);
            Rx_WLs[i][j] = wl0 + channel_spacing*j + norm_dist(rng);
    ...
    vector< vector<double> > costMat(TRX_NUM, vector<double> (TRX_NUM, 0));
    vector<bool> usedFlagTx(TRX_NUM, false);
    vector<bool> usedFlagRx(TRX_NUM, false);
    ...
    for(int i = 0; i < TRX_NUM; i++)
    {
        ...
        {
            if(i == j) continue;
            costMat[i][j] = tuning_dist(Tx_WLs[i], Rx_WLs[j], channel_spacing);
        }
    }
    ...
    for(int i = 0; i < TRX_NUM; i++)
    {
        ...
        for(int i = 0; i < TRX_NUM; i++)
        {
            if(usedFlagTx[i]) continue;
            for(int j = 0; j < TRX_NUM; j++)
            {
                if(i == j || usedFlagRx[j]) continue;
    ...
    usedFlagTx[currPairIndex.first] = true;
    usedFlagRx[currPairIndex.second] = true;
           
        printf("(%d, %d): %f\n", currPairIndex.first, currPairIndex.second, tuning_dist(Tx_WLs[currPairIndex.first], Rx_WLs[currPairIndex.second], channel_spacing));
    }
